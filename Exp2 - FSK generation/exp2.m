...
%{
    ...
    a carrier signal. The simplest FSK is binary FSK (BFSK). BFSK uses a pair
    of discrete frequencies to transmit binary (0s and 1s) information.
%}
...
fc1= 100 %input('Enter the freq of 1st Sine Wave carrier:');
fc2= 30 %input('Enter the freq of 2nd Sine Wave carrier:');
fp= 5 %input('Enter the freq of Periodic Binary pulse (Message):');
amp= 4 %input('Enter the amplitude (For Both Carrier & Binary Pulse Message):');
amp=amp/2;
...
%% Conclusion
%{
    Hence, we can observe that when the signal value is 1 then the carrier signal-1 was transmitted; otherwise, a signal value is 1 then the carrier signal-2 was transmitted. 
%}